...
exports.processRecurringTasks = void 0;
 ...
 */
const isRecurringDue = (rule, lastDate) => {
    const now = new Date();
...
const computeNextDueDate = (rule, baseDueDate) => {
    if (!baseDueDate)
        return null;
...
const processRecurringTasks = async () => {
    try {
        ...
        const recurringTasks = await db_1.default.task.findMany({
            where: {
        ...
        for (const task of recurringTasks) {
            ...
            if (!isRecurringDue(task.recurringRule, task.lastRecurringDate))
                continue;
...
exports.processRecurringTasks = processRecurringTasks;
//# sourceMappingURL=recurringTask.service.js.map